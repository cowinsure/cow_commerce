  ...
  const handleShare = async () => {
    if (navigator.share) {
      await navigator.share({
        title: cow.name,
        text: `Check out this ${cow.breed} - ${cow.name}`,
        ...
        >
          ...
          <div className="absolute inset-0 z-0">
            <Image
              ...
              src={cow.images[activeImage] || cow.image}
              priority
          ...
          {cow.images.length > 1 && (
            <>
              <button 
                onClick={(e) => {
                  e.stopPropagation();
                  setActiveImage((prev) => (prev === 0 ? cow.images.length - 1 : prev - 1));
                }}
              ...
              <button 
                onClick={(e) => {
                  e.stopPropagation();
                  setActiveImage((prev) => (prev === cow.images.length - 1 ? 0 : prev + 1));
                }}
          ...
          <div className="absolute bottom-6 right-6 z-20 px-3 py-1.5 rounded-full bg-black/50 backdrop-blur-sm text-white text-sm font-medium">
            {activeImage + 1} / {cow.images.length}
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-2">
        {cow.images.map((img, index) => (
          <button
            ...
            className={cn(
              'flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border-2 transition-all',
              activeImage === index
