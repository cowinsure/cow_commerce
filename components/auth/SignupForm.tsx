...
const signupSchema = z.object({
  ...
  ranchName: z.string().min(2, 'Ranch name must be at least 2 characters'),
...
export function SignupForm({ className }: { className?: string }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={cn('space-y-6', className)}>
      {/* Ranch Name Input */}
      <div className="group">
        <label className="block text-xs font-semibold text-on-surface-variant uppercase tracking-widest mb-2 px-1" htmlFor="ranch-name">
          Ranch Name
        </label>
        <div className="relative">
          <input
            id="ranch-name"
            type="text"
            placeholder="Emerald Valleys Livestock"
            ...
            className={cn(
              ...
              errors.ranchName ? 'border-error focus:border-error' : 'border-outline-variant focus:border-primary'
            )}
            {...register('ranchName')}
          />
        </div>
        {errors.ranchName && (
          <p className="mt-2 text-xs text-error px-1">{errors.ranchName.message}</p>
        )}
      </div>

      {/* Email Input */}
      <div className="group">
        <label className="block text-xs font-semibold text-on-surface-variant uppercase tracking-widest mb-2 px-1" htmlFor="signup-email">
          Email Address
        </label>
        <div className="relative">
          <input
      ...
      {/* Password Input */}
      <div className="group">
        <label className="block text-xs font-semibold text-on-surface-variant uppercase tracking-widest mb-2 px-1" htmlFor="signup-password">
          Password
        </label>
        <div className="relative">
          <input
      ...
      <div className="group">
        <label className="block text-xs font-semibold text-on-surface-variant uppercase tracking-widest mb-2 px-1" htmlFor="confirm-password">
          Confirm Password
        </label>
        <div className="relative">
          <input
        ...
        {errors.confirmPassword && (
          <p className="mt-2 text-xs text-error px-1">{errors.confirmPassword.message}</p>
        )}
      </div>
      ...
      <div className="flex items-start gap-3 py-2">
        <input
          ...
          type="checkbox"
          required
          className="w-5 h-5 rounded border-outline-variant text-primary focus:ring-primary-container mt-0.5"
        />
        <label htmlFor="terms" className="text-sm font-medium text-on-surface-variant leading-relaxed">
          I agree to the{' '}
          <a href="#" className="text-on-surface font-semibold underline decoration-primary-fixed underline-offset-2">Terms of Service</a>
          {' '}and{' '}
          <a href="#" className="text-on-surface font-semibold underline decoration-primary-fixed underline-offset-2">Privacy Policy</a>
        </label>
      </div>
